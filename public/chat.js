...
const systemPrompt = "Eres un programador.";

// A function to create a message element based on the text and alignment
function createMessageElement(text, alignment) {
    const messageElement = document.createElement("div");

    messageElement.className = `inline-block my-2.5 p-2.5 rounded border ${alignment === "left" ? "self-start bg-white" : "self-end bg-blue-200"
        }`;
...
function connectWebSocket(message, initChat) {
    ...
    websocket.addEventListener("open", () => {
        websocket.send(
            JSON.stringify({
                ...
                message: initChat ? "Dame la bienvenida como programador" : message, // Change the welcome message here
            })
        );
    });
    ...
    websocket.onclose = (event) => {
        if (event.code === 1000) {
            ...
            chatbox.scrollTop = chatbox.scrollHeight;
            receiving = false;
            sendButton.textContent = "Pregunta";
        }
    };
}
